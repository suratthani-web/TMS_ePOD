...
export async function createVehicle(data: VehicleFormData) {
  ...
  const { error } = await supabase
    ...
    .upsert({
      ...
      // Mileage columns are integers — round so decimal odometer readings
      // (e.g. "225341.3") don't hit "invalid input syntax for type integer".
      Current_Mileage: Math.round(Number(data.Current_Mileage) || 0),
      Next_Service_Mileage: Math.round(Number(data.Next_Service_Mileage) || 0),
